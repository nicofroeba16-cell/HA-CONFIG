#!/usr/bin/env python3
"""Idempotent runtime patch for HA dashboards and Apple Optik v1.9.28."""
...
def patch_js(path: Path) -> str:
    ...
    # iOS Light 1.0.1: bulb tile tap toggles immediately, long-press opens more-info.
    t = t.replace(
        '/* ===== light-card ===== */\n(function () {\nconst VERSION = "1.0.0";',
        '/* ===== light-card ===== */\n(function () {\nconst VERSION = "1.0.1";',
        1,
    )
    old_tile = '''    this._tile.addEventListener("click", (e) => {
      e.stopPropagation();
      this._more();
    });'''
    new_tile = '''    let tileHoldTimer = 0;
    ...
    # Remove the obsolete Apple touch icon reference when no real asset exists.
    old_touch_icon = '''  if (!document.querySelector('link[rel="apple-touch-icon"][data-apple-optik]')) {
...
'''
    if old_touch_icon in t:
        t = t.replace(old_touch_icon, "", 1)

    # Scope the global wash/background to Lovelace dashboards only.
    if "html::before" in t or "html::after" in t or "html.apple-wash-animating" in t:
        # Normalize standalone legacy selectors wherever they occur, including
        # comma-separated or same-line rules. Scoped selectors do not contain
        # the exact legacy token and are therefore left untouched on reruns.
...
html[data-panel=\"admin\"] ha-drawer {
  background: var(--primary-background-color, #111) !important;
    ...
    # Add route tracking only for legacy Apple Optik builds.
    # Current 1.9.25 already ships the standalone data-view/data-panel helper.
    standalone_route_helper = "data-view for per-view backgrounds (standalone)" in t
    if not standalone_route_helper:
